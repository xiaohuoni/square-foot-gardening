...
const createRow = (length) =>
  new Array(length).fill().map((cell) => new Plot());
const createGrid = (numRows, numCols) =>
  Array(numRows)
    .fill()
    .map((row) => createRow(numCols));
...
class Designer extends React.Component {
  ...
  addRowOptions() {
    ...
    return range(this.state.numRows + 1).map((index) => {
      return {
        label: label(index),
  ...
  deleteRow(index) {
    this.setState((state) => {
      return {
        ...
        grid: state.grid
          .map((row, rowNum) => (rowNum === index ? null : row.slice()))
          .filter(Boolean),
      };
    });
  }
  ...
  deleteRowOptions() {
    ...
    return range(this.state.numRows).map((index) => {
      return {
        label: label(index),
  ...
  addColOptions() {
    ...
    return range(this.state.numCols + 1).map((index) => {
      return {
        label: label(index),
    ...
    this.setState((state) => {
      return {
        ...
        grid: state.grid.map((row) =>
          row
            .map((plot, colNum) => (colNum === index ? null : plot))
            .filter(Boolean),
        ),
      };
    });
  ...
  deleteColOptions() {
    const label = (index) => String(index + 1);
    return range(this.state.numCols).map((index) => {
      return {
        label: label(index),
  ...
  renderPlantPicker() {
    if (this.state.showPlantPicker) {
      ...
      return (
        <PlantPicker
          show={true}
          handleSelect={(plant, layout) => {
            this.handlePlantPickerSelection(plant, layout);
          }}
          handleHide={() => {
            this.setState({ showPlantPicker: false });
          }}
