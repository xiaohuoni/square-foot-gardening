...
class SquareFootPlot extends React.Component {
  renderPlants() {
    if (this.props.plant) {
      ...
      const gridCellStyle = (row, col) => {
        return { gridArea: `${row + 1} / ${col + 1} / auto / auto` };
      };
      return this.props.layout.fill.map(([row, col], i) => {
        return (
          <div className="plant" style={gridCellStyle(row, col)} key={i}>
  ...
  render() {
    return (
      <button className="plot" id={this.props.id} onClick={this.props.onClick}>
        <div
          className="plants"
