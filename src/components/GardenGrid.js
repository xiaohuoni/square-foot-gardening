...
const propTypes = {
  grid: PropTypes.arrayOf(PropTypes.arrayOf(PropTypes.instanceOf(Plot)))
    .isRequired,
...
class GardenGrid extends React.Component {
  gridContainerStyles() {
    const sqSizeToShowAllColumns = Math.floor(
      (window.innerWidth * 0.9) / this.props.numCols,
    );
    const sqSizeToShowAllRows = Math.floor(
      (window.innerHeight * 0.7) / this.props.numRows,
    );
    const sqSizeToShowEverything = Math.min(
      sqSizeToShowAllColumns,
      sqSizeToShowAllRows,
    );
  ...
  render() {
    return (
      <div id="gridContainer" style={this.gridContainerStyles()}>
        <div id="grid" style={this.gridStyles()}>
